...
class VitTransformerTimm(nn.Module):
    ...
    Args:
        ...
        patch_size (int | tuple(int)): fixed Windows to split input images. Default 16
        in_chans (int): Number of input image channels. Default: 3
        num_classes (int): Number of classes for classification head. Default: 14
        embed_dim (int): Q,K,V matrix dimensions. Default: 768
        depths (int): Depth dimension. Default: 12
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=14,
                 embed_dim=768, depths=12, num_heads=12, mlp_ratio=4., qkv_bias=True, qk_norm=False,
                   num_mlp_heads=3, **kwargs):
